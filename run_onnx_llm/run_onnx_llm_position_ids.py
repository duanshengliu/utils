...
class BaseLLM:
    def __init__(self, onnx_file):
        ...
        if not onnx_file.endswith(".onnx"):
            logger_error(f"`{onnx_file}` is not a onnx file.")
    ...
    def _prepare_input(self, inputs, outputs, use_kv_cache=False):
        ...
        if use_kv_cache:
            ...
            if "position_ids" in inputs:
                inputs["position_ids"] = np.array(self.pos).reshape((1, 1))
            ...
            inputs["attention_mask"][:, self.pos] = 1
            if "position_ids" in inputs:
                inputs["position_ids"][:, self.pos] += inputs["position_ids"][:, self.pos - 1]
        return inputs
...
class PromptProcessor(BaseLLM):
    ...
    def pad_to_max_tokens(self, inputs, eos_token_id, pad_token_id=0, max_tokens=256, has_position_ids=False):
        # This pad function only supports batch_size = 1.
        input_ids = inputs["input_ids"]
        if input_ids.shape[0] != 1:
            logger_error(f"The batch size({input_ids.shape[0]}) is not equal to 1.")

        attention_mask = inputs["attention_mask"]
        if self.pos is None:
            self.pos = input_ids.shape[1]
        if max_tokens < self.pos:
            logger_error(f"The max_tokens({max_tokens}) < prompt token({self.pos}). "
                         f"Please consider increasing max_tokens!")
        ...
        if has_position_ids:
            pad_position_ids = np.cumsum(pad_attention_mask, axis=-1) - 1
            pad_position_ids[pad_attention_mask == 0] = 1
            pad_inputs["position_ids"] = pad_position_ids
        return pad_inputs
...
def main(
    ...
    if use_kv_cache and not model_with_kv_path:
        logger_error(f"`use_kv_cache` is True while `model_with_kv_path` is {model_with_kv_path}.")
    ...
    for inputs_meta in prompt_processor.llm_session._inputs_meta:
        if inputs_meta.name == "input_ids":
            ...
            if isinstance(seq_len, str):
                ...
                max_tokens = seq_len
        elif inputs_meta.name == "position_ids":
            has_position_ids = True
            logger.info("Detected position_ids in onnx model!")
        elif inputs_meta.name != "attention_mask":
            logger_error(f"Detected {inputs_meta.name} not in [input_ids, attention_mask, position_ids]!")
